...
export async function getHolidaySetForYear(
  apiBase: string,
  year: number
): Promise<Set<string>> {
  const cached = safeJsonParse<HolidayCache>(
    localStorage.getItem(keyForYear(year))
  );
  if (
    cached &&
    cached.year === year &&
  ...
  if (out.size > 0) {
    const payload: HolidayCache = {
      year,
      dates: Array.from(out),
      fetchedAt: Date.now(),
    };
...
export async function isMarketHoliday(
  apiBase: string,
  ymd: string
): Promise<boolean> {
